...
function passThrough(src) {
    ...
    cv.findContours(dstC4, contours, hierarchy, cv.RETR_TREE, cv.CHAIN_APPROX_SIMPLE, { x: 0, y: 0 });
    dstC3.delete();
    ...
    for (let i = 0; i < contours.size(); ++i) {
        ...
        if (c >= 2) {
            ...
            let x = rect.x;
            let y = rect.y;
            let radius = (rect.x + rect.width) / 8;
            let area = cv.contourArea(cnt, false);
            if (area > 100 && area < 3600) {
                cv.rectangle(dstC3, point1, point2, rectangleColor, 2, cv.LINE_AA, 0);
                cv.drawContours(dstC3, contours, i, color, 1, cv.LINE_8, hierarchy);
                xPointMean += x;
                yPointMean += y;
                radiusMean += radius;
                cv.circle(dstC3, new cv.Point(x, y), radius, color, thickness = 1);
                renderEarth(x / width, y / height, radius / 128);
            }
        }
    }
    contours.delete();
...
function renderEarth(x, y, radius) {
...
function backprojection(src) {
    ...
    let baseVec = new cv.MatVector(),
        targetVec = new cv.MatVector();
    baseVec.push_back(base);
    targetVec.push_back(dstC3);
    let mask = new cv.Mat(),
        hist = new cv.Mat();
    let channels = [0],
        histSize = [50];
...
function processVideo() {
    ...
    switch (controls.filter) {
        ...
        case 'adaptiveThreshold':
            result = adaptiveThreshold(src);
            break;
        case 'gaussianBlur':
            result = gaussianBlur(src);
            break;
        case 'bilateralFilter':
            result = bilateralFilter(src);
            break;
        case 'medianBlur':
            result = medianBlur(src);
            break;
        case 'sobel':
            result = sobel(src);
            break;
